...
Connect to review workflow management with live data
"""
...
def load_weekly_pool_data(review_type: str, tenant: str, week_ending: str) -> pd.DataFrame:
    """
    Load weekly pool data - new NDCs not in master tables
    """
    try:
        ...
        logger.info(f"DEBUG: Searching for {review_type} data between {week_start_date.strftime('%Y-%m-%d')} and {week_ending}")
        
        if review_type.lower() == 'fmt':
        ...
        if recent_count == 0:
            logger.warning(f"No recent FDB data found. Using sample of latest 20 records for testing.")
            if review_type.lower() == 'fmt':
                new_ndcs_query = f"""
                SELECT DISTINCT f.ndc, f.brand, f.gsn, f.hic3, f.mfr, 
                       f.load_date, 'NEW' as status,
        ...
        logger.info(f"Weekly {review_type.upper()} pool for {tenant} week ending {week_ending}: {len(df)} new NDCs")
        if not df.empty:
        ...
        logger.error(f"Error loading weekly {review_type} pool data: {e}")
        return pd.DataFrame()

def get_review_assignments(review_type: str, tenant: str, week_ending: str) -> Dict[str, List[Dict]]:
    """
    Get current reviewer assignments for the week
    """
    try:
        from backend.services.connector import query
        from backend.config.settings import get_settings
        ...
        # Query review assignments table (would need to be created)
        assignments_query = f"""
        SELECT ndc, reviewer, mbid, eff_date, end_date, status, assignment_date
        FROM demo.gainwell.{review_type}_review_assignments 
        ...
        logger.error(f"Error loading {review_type} assignments: {e}")
        return {'reviewer_a': [], 'reviewer_b': []}
...
async def get_weekly_pool(
    ...
    try:
        ...
        logger.info(f"Weekly {review_type} pool: tenant={tenant}, week_ending={week_ending}, user={user_email}")
        
        # Validate inputs
        if review_type.lower() not in ['fmt', 'pdl']:
            raise HTTPException(status_code=400, detail="Review type must be 'fmt' or 'pdl'")
        
        # Load weekly pool data
        df = load_weekly_pool_data(review_type, tenant, week_ending)
        ...
        logger.error(f"Error getting weekly {review_type} pool: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to get weekly pool: {str(e)}")

@router.get("/groups/{review_type}")
async def get_review_groups(
    request: Request,
    review_type: str,
    tenant: str = Query(..., description="Tenant (MASTER/AK/MO)"),
    week_ending: str = Query(..., description="Week ending date (YYYY-MM-DD)")
    ...
    """
    Get review groups categorized by match type with status counts
    
    Example: GET /api/weekly/groups/fmt?tenant=AK&week_ending=2024-12-15
    """
    try:
        user_email = request.headers.get("X-Forwarded-Email", "unknown")
        logger.info(f"Review groups: {review_type}, tenant={tenant}, week_ending={week_ending}, user={user_email}")
        ...
        assignments = get_review_assignments(review_type, tenant, week_ending)
        assignment_map = {}
        ...
        logger.error(f"Error getting review groups: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to get review groups: {str(e)}")

@router.get("/assignments/{review_type}")
async def get_reviewer_assignments(
    ...
    Example: GET /api/weekly/assignments/fmt?tenant=AK&week_ending=2024-12-15
    """
    try:
        user_email = request.headers.get("X-Forwarded-Email", "unknown")
        logger.info(f"Getting {review_type} assignments: tenant={tenant}, week_ending={week_ending}, user={user_email}")
...
@router.post("/assign")
async def assign_reviews(request: Request):
    """
    Assign NDCs to reviewers with MBID and dates
    ...
    try:
        ...
        user_email = request.headers.get("X-Forwarded-Email", "unknown")
        logger.info(f"Assigning {len(assignments)} {review_type} reviews: tenant={tenant}, user={user_email}")
        
        # TODO: Insert assignments into database table
        # For now, return success - would need to create assignment tables:
        # demo.gainwell.fmt_review_assignments / demo.gainwell.pdl_review_assignments
        
        return {
            "message": f"Successfully assigned {len(assignments)} NDCs for review",
            "review_type": review_type,
            "tenant": tenant,
            "week_ending": week_ending,
        ...
        logger.error(f"Error assigning reviews: {e}")
        raise HTTPException(status_code=500, detail=f"Assignment failed: {str(e)}")

@router.post("/resolve")
async def resolve_conflicts(request: Request):
    """
    Resolve conflicts between reviewer assignments
    ...
    try:
        ...
        user_email = request.headers.get("X-Forwarded-Email", "unknown")
        logger.info(f"Resolving {len(resolutions)} {review_type} conflicts: tenant={tenant}, user={user_email}")
        
        # TODO: Update resolution status in database
        
        return {
            "message": f"Successfully resolved {len(resolutions)} conflicts",
            "review_type": review_type,
            "tenant": tenant,
            "week_ending": week_ending,
...
@router.post("/approve")
async def approve_weekly_review(request: Request):
    """
    Final approval - sync resolved items to master tables
    ...
    try:
        ...
        user_email = request.headers.get("X-Forwarded-Email", "unknown")
        logger.info(f"Approving {len(approved_items)} {review_type} items: tenant={tenant}, user={user_email}")
        
        # TODO: Insert approved items into master tables
        # For FMT: demo.gainwell.fmt_mbids
        # For PDL: demo.gainwell.pdl_keycodes
        
        return {
            "message": f"Successfully approved and synced {len(approved_items)} items to master",
            "review_type": review_type,
            "tenant": tenant,
            "week_ending": week_ending,
        ...
        logger.error(f"Error approving weekly review: {e}")
        raise HTTPException(status_code=500, detail=f"Approval failed: {str(e)}")

@router.post("/reject")
async def reject_items(request: Request):
    """
    Reject selected items from review
    ...
    try:
        ...
        user_email = request.headers.get("X-Forwarded-Email", "unknown")
        logger.info(f"Rejecting {len(rejected_ndcs)} {review_type} NDCs: tenant={tenant}, user={user_email}")
        
        # TODO: Mark items as rejected in database
        
        return {
            "message": f"Successfully rejected {len(rejected_ndcs)} NDCs",
            "review_type": review_type,
            "tenant": tenant,
            "week_ending": week_ending,
...
@router.get("/comparison/{review_type}")
async def get_comparison_data(
    ...
    """
    Get comparison data for conflict resolution between reviewers
    
    Example: GET /api/weekly/comparison/fmt?tenant=AK&week_ending=2024-12-15
    """
    try:
        user_email = request.headers.get("X-Forwarded-Email", "unknown")
        logger.info(f"Getting {review_type} comparison data: tenant={tenant}, week_ending={week_ending}, user={user_email}")
